...
/** @Author: Bryce @Date: 2020/10/29 21:33 */
public class CountSqlParser implements ISqlParser {
    private static final List<SelectItem> COUNT_SELECT_ITEM =
            Collections.singletonList(defaultCountSelectItem());
    ...
    public SqlInfo parser(MetaObject metaObject, String sql) {
        ...
        try {
            ...
            // #95 Github, selectItems contains #{} ${}, which will be translated to ?, and it may
            // be in a function: power(#{myInt},2)
            for (SelectItem item : plainSelect.getSelectItems()) {
                if (item.toString().contains(StringPool.QUESTION_MARK)) {
                    return sqlInfo.setSql(
                            MySqlParserUtil.getOriginalCountSql(selectStatement.toString()));
                }
            }
            ...
            // 无法优化使用原 SQL
            return sqlInfo.setSql(MySqlParserUtil.getOriginalCountSql(sql));
        }
    }
}
