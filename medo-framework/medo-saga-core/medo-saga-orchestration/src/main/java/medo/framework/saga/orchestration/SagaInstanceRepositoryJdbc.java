...
public class SagaInstanceRepositoryJdbc implements SagaInstanceRepository {
    ...
    public SagaInstanceRepositoryJdbc(JdbcTemplate jdbcTemplate, IdGenerator idGenerator) {
        this.jdbcTemplate = jdbcTemplate;
        this.idGenerator = idGenerator;

        String sagaInstanceTable =
                "saga_instance"; // eventuateSchema.qualifyTable("saga_instance");
        String sagaInstanceParticipantsTable =
                "saga_instance_participants"; // eventuateSchema.qualifyTable("saga_instance_participants");

        insertIntoSagaInstanceSql =
                String.format(
                        "INSERT INTO %s(saga_type, saga_id, state_name, last_request_id, saga_data_type, saga_data_json, end_state, compensating) VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
                        sagaInstanceTable);
        insertIntoSagaInstanceParticipantsSql =
                String.format(
                        ...
                        (rs, rownum) ->
                                new SagaInstance(
                                        ...
                                        new SerializedSagaData(
                                                rs.getString("saga_data_type"),
                                                rs.getString("saga_data_json")),
