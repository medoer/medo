...
public class SagaInstanceFactory {
    ...
    public <SagaData> SagaInstance create(Saga<SagaData> saga, SagaData data) {
        ...
        if (sagaManager == null) throw new RuntimeException(("No SagaManager for " + saga));
        return sagaManager.create(data);
    }

    private <SagaData> SagaManager<SagaData> makeSagaManager(
            SagaManagerFactory sagaManagerFactory, Saga<SagaData> saga) {
